...
class SecretaresseModel {
    ...
    public function getDirecteuren()
    {
       $sql = 'SELECT `contacten`.* FROM  `contacten` WHERE `contacten`.`recht`=\'directeur\' ';
       $stmnt = $this->db->prepare($sql);
       $stmnt->execute();
       $directeuren = $stmnt->fetchAll(\PDO::FETCH_CLASS,__NAMESPACE__.'\db\Contact');    
       return $directeuren;
    }
   ...
   public function getGebruiker()
   {
     ...
     public function getAndereGebruiker()
   {
         $id =  filter_input(INPUT_GET,'id');
        $sql = "SELECT `contacten`.* FROM  `contacten` WHERE `contacten`.`id`=$id ";
       $stmnt = $this->db->prepare($sql);
       $stmnt->execute();
       $gebruiker = $stmnt->fetchAll(\PDO::FETCH_CLASS,__NAMESPACE__.'\db\Contact');    
       return $gebruiker;
      // gebruiker ID pakken en ophalen uit db. in model proppen en returnen.
...
public function wijzigAndereGebruiker(){
    ...
    public function wijzigAnw()
    {
        ...
        //if($gebruikersnaam!=$this->getGebruiker()->getGebruikersnaam())
        {
            $sql="UPDATE `contacten` SET gebruikersnaam=:gebruikersnaam,voorletter=:voorletter"
                . ",tussenvoegsel=:tussenvoegsel,achternaam=:achternaam,"
                 . "extern=:extern,intern=:intern,email=:email where `contacten`.`id`= $gebruiker_id; ";
    ...
    public function resetWw() {
         $ww = 'qwerty';
        $id =  filter_input(INPUT_GET,'id');
        $sql = "UPDATE `contacten` SET `contacten`.`wachtwoord` = :ww WHERE `contacten`.`id`= $id";
        $stmnt = $this->db->prepare($sql);
    ...
    public function wijzigFoto() {
        //bedenk een naam voor de foto.
        $fotoNaam = FOTO::getAfbeeldingNaam();
        
        //sla foto op
        $result = FOTO::slaAfbeeldingOp($fotoNaam);
        if($result === false){
