...
//        $this->view->set('afdelingen', $afdelingen); -- niet nodig
    }
    private function dicbeheerAction()
    {
        $gebruiker = $this->model->getGebruiker();
        $this->view->set('gebruiker',$gebruiker);
        
        
        $directeuren = $this->model->getDirecteuren();
        $this->view->set('directeuren', $directeuren);
    ...
    private function resetwwAction(){
       $gebruiker = $this->model->getGebruiker(); 
        $this->view->set('gebruiker',$gebruiker);
            ...
            switch($result){
                case REQUEST_SUCCESS:
                    $this->view->set('boodschap','wijziging wachtwoord van andere leraar is gelukt');
                    break;
                case REQUEST_FAILURE_DATA_INVALID:
                    $this->view->set("boodschap","nieuwe wachtwoord niet identiek of oude wachtwoord fout. Poog opnieuw!");
                    break;
    ...
    private function wijziggebruikersAction(){
         if($this->model->isPostLeeg())
        {
           $this->view->set("boodschap","Wijzig hier de andere gebruiker zijn gegevens");
        }
        else{
            $result = $this->model->wijzigAndereGebruiker();
            switch($result){
                case REQUEST_SUCCESS:
                    $this->view->set('boodschap','wijziging gelukt');
                    break;
        ...
        $gebruiker = $this->model->getAndereGebruiker();
